...
// Final remaining mappings
const MODULE_MAP = {
    // Components
    'HealthComponent': 'components/HealthComponent',
    'StatsComponent': 'components/StatsComponent',
    'CombatComponent': 'components/CombatComponent',
    'InventoryComponent': 'components/InventoryComponent',
    'AIComponent': 'components/AIComponent',

    // AI
    'EnemyAI': 'ai/behaviors/enemies/EnemyAI',
    'BossAI': 'ai/behaviors/bosses/BossAI',
    'NPCAI': 'ai/behaviors/npcs/NPCAI',
    'BaseAI': 'ai/behaviors/BaseAI',

    // Systems
    'HeroSystem': 'systems/HeroSystem',

    // Gameplay
    'Boss': 'gameplay/Boss',

    // Config
    'HeroDefaults': 'config/HeroDefaults',
    'SpeciesScaleConfig': 'config/SpeciesScaleConfig',
...
function processFile(filePath) {
    ...
    while ((match = declareRegex.exec(content)) !== null) {
        ...
        if (MODULE_MAP[name]) {
            ...
            const fullPath = path.join(SRC_DIR, MODULE_MAP[name] + '.ts');
            if (fs.existsSync(fullPath)) {
                declares.push({ name, fullMatch: match[0] });
            } else {
                console.log(`  Warning: ${MODULE_MAP[name]} not found for ${name}`);
            }
        }
    }
...
console.log('Phase 4 - Final module fixes...');
walkDir(SRC_DIR);
