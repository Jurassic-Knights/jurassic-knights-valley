---
name: coding-guide
description: Master architecture principles - ECS, EventBus, data-driven design, performance. Use when implementing game systems, reviewing architecture, or following project coding standards.
---
...
This skill is the **single source of truth** for architecture. A short pointer document exists at `documents/design/master_principles.md`.
...
1. Check `documents/design/art_guide.md` for art direction (and `documents/design/lore_guide.md`, `documents/design/sound_guide.md` for copy/audio)
2. Check `documents/design/technical_guidelines.md` for ID naming
3. Check existing registry in `AssetLoader.ts` to avoid duplicate IDs
...
5. Add entry to `src/core/AssetLoader.ts` in the asset registry

### Creating an Audio Asset
1. Create/acquire the audio file
2. Save to appropriate folder in `assets/audio/`
3. Add entry to `src/core/AssetLoader.ts` in the audio registry

### ID Naming Rules
See `documents/design/asset_id_conventions.md` for the full table of patterns and rules; add new conventions there. Summary: `snake_case`, category prefix (`ui_`, `npc_`, entity type, `sfx_`, `bgm_`, etc.), generic IDs not display names; IDs are permanent.
