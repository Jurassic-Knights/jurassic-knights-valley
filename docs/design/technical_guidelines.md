...
**Max 500 lines per source file** to ensure maintainability and AI context optimization.

### Rules
- **Source files** (`.js`, `.css`) must not exceed 500 lines
- **Data files** (`.json`) are exempt from this limit
- **Generated files** (`asset_manifest.js`) are exempt
- **Vendor/library files** (`libs/`) are exempt

### Splitting Patterns

| Pattern | Use Case | Example |
|---------|----------|---------|
| **Species/Type** | Entity-specific handlers | `SFX_Dino_Tyrannosaur.js`, `SFX_Dino_Carnotaurus.js` |
| **Concern** | Class method groups | `EnemyCore.js`, `EnemyBehavior.js`, `EnemyRender.js` |
| **Component** | CSS modules | `ui-hud-core.css`, `ui-hud-panels.css` |
| **Tier** | Grouped by progression | `Enemies_T1.js`, `Enemies_T2.js` |

### Prototype Extension Pattern (JavaScript)

When splitting class methods across files, use prototype extension:

```javascript
// EnemyCore.js - Base class with constructor
class Enemy extends Entity {
    constructor(config) { /* ... */ }
}
window.Enemy = Enemy;

// EnemyBehavior.js - Extends prototype
Enemy.prototype.updateWander = function(dt) { /* ... */ };
Enemy.prototype.updateChase = function(dt) { /* ... */ };
```

**Load Order**: Core file MUST load before extension files in `index.html`.
