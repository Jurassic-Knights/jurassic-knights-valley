---
name: code-review
description: Thorough, constructive code review - correctness, security, performance, quality. Use when reviewing pull requests, examining code changes, or when the user asks for a code review.
---
...
### 9) Game Code (ECS / game loop)
For game systems and Canvas/game-loop code, also apply the **coding-guide** audit rubric:
- Modularity (EventBus, no direct system calls), scalability, no allocations in update loops, spatial partitioning (Quadtree) where appropriate, data-driven config (no magic numbers).
