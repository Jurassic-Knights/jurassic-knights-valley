...
class EquipmentManager {
    ...
    constructor(owner) {
        ...
        if (window.EquipmentSlotsConfig) {
            for (const slotId of EquipmentSlotsConfig.getSlotIds()) {
                this.slots[slotId] = null;
            }
        }

        // Active weapon set (1 = hand1/hand2, 2 = hand1_alt/hand2_alt)
    ...
    equip(slotId, equipmentData) {
        ...
        if (window.EquipmentSlotsConfig && !EquipmentSlotsConfig.canEquip(slotId, itemType)) {
     ...
     */
    getActiveSetBonuses() {
        if (!window.SetBonusesConfig) return [];
