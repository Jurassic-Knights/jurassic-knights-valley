...
const Profiler = {
    ...
    init() {
        ...
        this.enabled = window.ENV?.SHOW_FPS || false;
    ...
    update() {
        ...
        if (delta >= 500) {
            ...
            if (window.VFXController && VFXController.getActiveCount) {
                this.vfxCount = VFXController.getActiveCount();
            } else if (window.ParticleSystemManager) {
                this.vfxCount = ParticleSystemManager.getActiveCount?.() || 0;
            }

            // Get current zone
            if (window.IslandManager && window.Game?.hero) {
