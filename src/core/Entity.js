...
class Entity {
    ...
    drawShadow(ctx, forceOpaque = false) {
        ...
        if (window.RenderConfig && RenderConfig.Hero) {
            const shadowWidth = this.width * RenderConfig.Hero.SHADOW_SCALE_X;
            const shadowHeight = shadowWidth * RenderConfig.Hero.SHADOW_SCALE_Y;
        ...
        const shadowWidth = this.width; // Use full width
        const shadowHeight = this.height; // Use full height as base
        ...
        let shadowImg = null;
        if (this.sprite && window.MaterialLibrary) {
        ...
        } else if (this.sprite && window.AssetLoader) {
