...
const AssetLoader = {
     ...
     * Priority: EntityRegistry â†’ static assets â†’ pattern-based fallback â†’ placeholder
    ...
    _getEntityImagePath(id) {
        if (!window.EntityRegistry) return null;
        ...
        for (const category of categories) {
            const registry = window.EntityRegistry[category];
            ...
            // Try with category prefix (e.g., "dinosaur_t1_01" â†’ "enemy_dinosaur_t1_01")
            // This handles sprite ID lookups
        }

        // Special case: hero
        if (id === 'hero' && window.EntityRegistry.hero?.files) {
            return this._selectBestPath(window.EntityRegistry.hero.files);
        }
     ...
     * Priority: clean â†’ approved_original â†’ original
     ...
     */
    getVFXPreset(id) {
        return window.VFXController?.presets?.[id] || null;
    }
};
