...
export class ZoneSystem {
    ...
    public async setZones(
        ...
        this._applyRawUpdates(rawUpdates, worldData, chunksToUpdate, dirtyGroundTiles);

        // 1b. Coast sync: add/remove terrain_coast on grasslands adjacent to water
        this._processCoastUpdates(worldData, chunksToUpdate, dirtyGroundTiles);

        // 1c. Splats for coast tiles (grass+sand blend)
        const allSplatChanges = await this._evaluateSplats(dirtyGroundTiles, worldData, loadedChunks);
        ...
        await this._updateGroundTextures(dirtyGroundTiles, allSplatChanges, worldData, loadedChunks);
    ...
    private async _updateGroundTextures(
        dirtyGroundTiles: Map<string, Set<string>>,
        allSplatChanges: { chunkKey: string; idx: number; oldVal: number; newVal: number }[],
        worldData: Map<string, ChunkData>,
        loadedChunks: Map<string, PIXI.Container>
    ) {
