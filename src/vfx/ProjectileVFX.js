...
const ProjectileVFX = {
    ...
    spawn(origin, target, weaponType = 'pistol') {
        ...
        if (window.RenderConfig?.Hero?.WEAPON?.RIFLE) {
    ...
    update(dt) {
        ...
        for (let i = this.projectiles.length - 1; i >= 0; i--) {
            ...
            if (p.active && p.glow && window.LightingSystem && typeof window.LightingSystem.addLight === 'function') {
                try {
                    ...
                    window.LightingSystem.addLight(p.x, p.y, radius, p.color, p.alpha, p.angle, elongation);
                } catch (e) {
