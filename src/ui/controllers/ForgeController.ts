...
class ForgePanel extends UIPanel {
    ...
    render(view = 'dashboard', context: ForgeContext = {}) {
        ...
        if (view === 'dashboard') {
            this._renderDashboard(grid);
        } else if (view === 'recipes') {
            this._renderRecipes(grid, context);
        } else if (view === 'batch') {
            this._renderBatch(grid, context);
        }
    }

    private _renderDashboard(grid: Element) {
    ...
    private _renderRecipes(grid: Element, context: ForgeContext) {
        // Check context
    ...
    private _renderBatch(grid: Element, context: ForgeContext) {
