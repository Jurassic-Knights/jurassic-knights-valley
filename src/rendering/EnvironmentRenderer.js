...
class EnvironmentRenderer {
    constructor() {
        ...
        this.lightning = {
            timer: 0,
            flashAlpha: 0,
            bolt: null, // { points: [], life: 0 }
            boltColor: '#ffffff'
        };

        // Wind State (Gust System)
        this.wind = {
            currentX: 0, // Current wind force X
            targetX: 0, // Target wind force (for lerping)
            baseX: 10, // Gentle breeze always present
            gusting: false,
            timer: 0 // Timer for next state change
        };
        ...
        this.lightingSchedule = [
            { time: 0.0, color: { r: 10, g: 10, b: 35 }, alpha: 0.85 }, // Midnight (Deep Blue)
            { time: 0.05, color: { r: 10, g: 10, b: 35 }, alpha: 0.7 }, // Dawn Start (still dark)
            { time: 0.1, color: { r: 255, g: 100, b: 50 }, alpha: 0.3 }, // Dawn (Orange sunrise)
            { time: 0.15, color: { r: 255, g: 255, b: 255 }, alpha: 0.0 }, // Day Start (Clear)
            { time: 0.7, color: { r: 255, g: 255, b: 255 }, alpha: 0.0 }, // Day End
            ...
            { time: 0.9, color: { r: 10, g: 10, b: 35 }, alpha: 0.75 }, // Night Start
            { time: 1.0, color: { r: 10, g: 10, b: 35 }, alpha: 0.85 } // Loop to Midnight
        ];
    ...
    init(game) {
        ...
        if (window.EventBus && window.GameConstants) {
            EventBus.on(GameConstants.Events.TIME_TICK, (data) => this.updateLighting(data));
            EventBus.on(GameConstants.Events.WEATHER_CHANGE, (data) => this.setWeather(data.type));
        }
    ...
    setWeather(type) {
        ...
        if (window.ProceduralSFX && window.ProceduralSFX.ctx) {
