...
class WorldRenderer {
    ...
    render(ctx, viewport) {
        ...
        if (timing) {
            timing.worldWater = (timing.worldWater || 0) + performance.now() - t0;
        }
        ...
        if (timing) {
            timing.worldIslands = (timing.worldIslands || 0) + performance.now() - t0;
        }
    ...
    drawWater(ctx, viewport) {
        ...
        // Lazy load image and create pattern
        ...
        // Create pattern from loaded image
        if (this._baseLayerLoaded && !this.backgroundPattern && this._baseLayerImg) {
            this.backgroundPattern = ctx.createPattern(this._baseLayerImg, 'repeat');
        }
    ...
    drawWorld(ctx, viewport) {
        ...
        for (const island of islandManager.islands) {
            ...
            if (
                ...
                island.worldX > vpRight ||
                island.worldY + island.height < vpTop ||
                island.worldY > vpBottom
            ) {
                continue;
            }
