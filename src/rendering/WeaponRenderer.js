...
const WeaponRenderer = {
    ...
    drawMeleeWeapon(ctx, hero, baseAngle, facingRight, spriteId, weaponSubtype = 'sword') {
        ...
        if (hero.isAttacking && hero.attackTimer > 0) {
            ...
            if (window.MeleeTrailVFX && progress >= 0.2 && progress <= 0.6) {
        ...
        const weaponPath = window.AssetLoader ? AssetLoader.getImagePath(spriteId) : null;
        if (weaponPath) {
    ...
    drawRifle(ctx, hero, baseAngle, facingRight = true, spriteId = 'weapon_ranged_pistol_t1_01') {
        ...
        const gunPath = window.AssetLoader ? AssetLoader.getImagePath(spriteId) : null;
        if (gunPath) {
    ...
    drawShovel(ctx, hero, baseAngle, facingRight = true, spriteId = 'tool_t1_01') {
        ...
        // Draw Sprite using spriteId
        const toolPath = window.AssetLoader ? AssetLoader.getImagePath(spriteId) : null;
        if (toolPath) {
